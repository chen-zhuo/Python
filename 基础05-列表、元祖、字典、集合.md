...
list3 = list(i * 2 for i in range(10) if i % 3 == 0)  
...
**`max/min` 函数**： `max/min(列表)`，获取列表中最大或最小的元素。
...
2. 字典里面所有数据以**键值对**的形式存储。
3. **键值对就是一个键对一个值，`{key：value}`，`key`为键， `:` 分隔符，`value`为值（保存数据）**。
4. **键(`key`)，是不可变的数据类型，一般使用字符串作为唯一的键（若键不唯一，后面的值会覆盖前面的值）**。
5. **值(`value`)，可以是任何类型的数据**。
6. 字典不能使用下标，因此数据是**无序**的。
7. `{} ` 表示空字典，即没有元素的字典。
...
元祖(`tuple`)是 `Python` 中用来存储数据的一个数据类型。

1. 元祖里面能存储**多个不同类型数据**，数据之间用 `,` 隔开，每个数据也可以叫**元素**。
2. **元祖使用 `()` 将元素包含起来，当只有一个元素的时候，后面要加逗号。**
3. 元祖里**元素的内容和个数是不能被改变的**。
4. 元祖可以使用下标，因此是**有序**的。
...
**拼接**：通过 `+` 符号，将两个元祖合并成一个元祖。

```python
print((1, 2)+(3, 4))    # (1, 2, 3, 4)
```

**重复输出**：通过 `*` 符号，将元祖里面的数据重复输出。

```python 
print((1, 2) * 2)       # (1, 2, 1, 2)
```

**判断包含**：`元素 in/not in 元祖`，判断元素是否包含在元祖中。
...
1. `变量1，变量2... = 元祖`，通过多个变量分别获取元祖元素（**变量和元素数量必须一样,不一样会报错**）。
2. `变量1，*变量2 = 元祖`，通过在变量名前加 `*` 可以把变量变成列表，获取多个元素。

```python
names = ('name1', 'name2', 'name3',)
x, y, z = names  	            # 变量个数和元祖里的元素的个数必须一样
...
1. 列表里面增加或删除元素，会改变该元素后面的其他元素的下标位置，并改变列表长度。
2. 列表不适合存储大数据，假如使用循环在列表中添加1亿个元素，内存绝对爆炸。
3. 字典中，键值对中的键和值必须同时存在，任何一方消失，整个键值对就没有了。
4. 字典中， `key` 确定存在，使用 `字典[key]` 获取值； `key` 不确存在，使用 `字典.get(key)` 获取值。