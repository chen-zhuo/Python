...
**UTF-8不需要BOM来表明字节顺序，但可以用BOM来表明编码方式。**字符"ZERO WIDTH NO-BREAK SPACE"的UTF-8编码是EF BB BF。所以如果接收者收到以EF BB BF开头的字节流，就知道这是UTF-8编码了。**所以这里UTF-8编码就有两种：UTF-8、带BOM的UTF-8**

**Windows系统有时又会有坑，在装机版Win10系统中，文本文件另存为时，就只有一个 `UTF-8`，而这个UTF-8编码是[带BOM的UTF-8]，而正版Win10系统中，文本文件另存为时，就会有两个 `UTF-8`，分别是 [UTF-8、带BOM的UTF-8]。**

![QQ截图20200907234514](image/QQ截图20200907234514.png)

![QQ截图20200908001101](image/QQ截图20200908001101.png)

下面来看看UTF-8、带BOM的UTF-8在Python程序中读取的区别：
...
# 注释：可以看到带BOM的UTF-8比UTF-8多了一个﻿，在列表中内容为\ufeff，编码中内容为\xef\xbb\xbf，当中ef bb bf就恰好对应了上面EF BB BF开头的字节流为带BOM的UTF-8。
```

去掉BOM有两种方法：

1. 打开文本文件，将其另存为**不带有BOM的UTF-8**。（最好是在正版系统中能显示有两个UTF-8编码选项下操作）
...
with open("Task.txt", "r", encoding='utf-8')as f:
    nr = f.readlines()
    nr = nr[0].encode().decode('utf-8-sig')      # 新华社 字符长度：3
    print(nr, f'字符长度：{len(nr)}')
```
