...
**NumPy数组：NumPy提供的最重要的数据结构，是Python数组的扩展。**
...
**Python没有数组，只有元组(tuple)和列表(list)，元组数据不可改变，列表数据可改变。这里的数组是指的 `NumPy数组`。**
...
前面提到“数组”是Numpy的特色，并且**许多底层函数实际上是用C编写，因此运算速度非常快，你可以体验在原生Python中永远无法体验到的速度。**
...
前面使用列表（list）创建了一个一维数组的方法，但还有其他方法：
...
'''
解释：上面的代码显示了创建数组的4种不同方法。最基本的方法是将序列传递给NumPy的array()函数; 你可以传递任何序列（类数组），而不仅仅是常见的列表（list）数据类型。
'''
```
...
如果创建一个2D（二维）数组，就要传递一个列表的列表（或者是一个序列的序列）给array()函数。

如果创建一个3D（三维）数组，就要传递一个列表的列表的列表。

如果创建一个4D（四维）数组，就要传递一个列表的列表的列表的列表，以此类推。
...
**`where()`函数**：根据条件返回数组中的值的有效方法。只需要把条件传递给它，它就会返回一个使得条件为真的元素的下标的列表。
